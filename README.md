...
- アプリの機能として、「自分で決めたことを三日間試してみる機会」と「他者と共有する場」を提供
...
- 三日坊主の登録
    - 概要、取り組む期間、カテゴリ、意気込み（必須）、詳細（任意）などを入力
        ...
        - 三日坊主が思い浮かばない人向けに、「概要」設定の入り口は「自分で選択」か「自動設定」を選べる。「自動設定」の場合は2~3個の質問に答えることで自動で設定される（あらかじめいくつか用意しておく）
    - 記録を促す通知の時間帯も設定する
        - デフォルトで決められているが、変えたい人は変えられる
        - 通知は、PWA化してプッシュ通知するイメージ
    - 設定した時点でXシェアできる
...
- 三日坊主一覧画面
    ...
    決めた内容を後から編集できてしまうのは「三日だけ集中して取り組む」という趣旨にそぐわないと考え、編集ボタンは設定しない旨を検討
...
- 日々の三日坊主の記録
    ...
    - 記録を促す通知は必須だと考えるため、設定した時間にプッシュ通知を送る。受け取った通知をクリックすると記録画面に遷移できる。
    - 三日目の記録が完了したら、「三日坊主を完了する」ボタンが表示されたポップアップが出てくる。クリックすると該当する三日坊主のステータスが完了済みになり、完了報告をXでシェアできる。また、次の三日坊主の設定を促される。
    
    以下はボツ案
    
    - アプリ上でも記録できるが、基本的には設定した時間に送られてくるLINEのメッセージに答える形で記録する。上記の「必須」項目それぞれに関する質問メッセージを受領し、メッセージ内のボタンを押すことで最低限の記録が完結する。より詳細に記録したい場合は、メッセージに含まれるリンクからアプリへ遷移する。
        - ユーザーの記録の手間を最小限に、と考えてLINE連携を検討したが、LINEを挟むことでアプリの世界観に現実が入り込んでしまうとも考えられる（コンセプトやターゲットユーザーはある程度定まってるので、それに合わせた世界観をアプリ内で表現したい）。
        - アプリの機能のみで完結させるとしても、手間やストレスを最小限にする工夫はできるはず。主に「通知からアプリに飛んでもらう」「飛んだ後に記録してもらう」の2点で工夫が必要だが、LINEじゃないといけない理由はなさそう。現状では、上記のとおりボタン選択＆スライド式の記録形式などを検討中。
